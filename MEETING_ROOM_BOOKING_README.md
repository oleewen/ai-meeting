...
3. **会议室预约**
   ...
   - 时长限制（30分钟~4小时）
   - 提前预约截止时间（开始前5分钟）
   - **AI生成会议议程**：根据会议主题自动生成Markdown格式的会议议程推荐
...
#### 3. 配置腾讯云混元大模型API（可选，用于AI生成议程功能）

编辑 `ai-meeting-boot/src/main/resources/application-dev.properties`，配置API密钥：
...
**注意**: 
- 如果不配置API密钥，AI生成议程功能将不可用，但不影响其他功能
- 如果使用其他环境（如test、pre、prod），请修改对应的配置文件
...
- `POST /api/bookings/agenda-suggestion` - AI生成会议议程推荐
...
系统集成了腾讯云混元大模型AI服务，可以根据会议主题自动生成结构化的会议议程，并自动填入备注字段，提升会议准备效率。

### 使用方法

1. **输入会议主题**
   - 在创建预约时，输入会议主题
   - 输入主题后，"AI生成议程"按钮会自动显示

2. **生成议程**
   - 点击"AI生成议程"按钮
   - 系统会调用AI服务生成Markdown格式的会议议程
   - 生成过程中会显示"生成中..."状态
...
1. **API配置**：使用前需要在配置文件中设置腾讯云混元大模型API密钥
2. **网络要求**：需要能够访问腾讯云API服务
3. **响应时间**：AI服务响应时间可能较长（通常3秒内），请耐心等待
4. **内容验证**：系统会自动验证生成内容的长度（50-500字符），不符合要求时会提示手动输入
5. **降级处理**：如果AI服务不可用或超时，系统会提示手动输入备注，不影响正常预约功能

### 错误处理

- **服务不可用**：显示"AI服务暂时不可用，请稍后重试或手动输入备注"
- **请求超时**：显示"请求超时，请稍后重试或手动输入备注"
- **内容不符合要求**：显示"生成的议程内容不符合要求，请手动输入备注"
